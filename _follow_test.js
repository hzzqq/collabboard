// _follow_test.js — CollabBoard follow（关注成员实时光标转发，离场自动清理）
...
const PORT = 8133;
const ROOM = 'foll_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_foll_inline.js');
...
(async ()=>{
  ...
  // c1 关注 c2
  c1.send({ type:'follow', toId:c2cid });
  await sleep(300);
  const f = c2.msgs.filter(m => m.type==='follow' && m.from===c1cid).pop();
  ok('c2 收到 follow 事件', !!f);
  ok('follow 目标= c2', f && f.toId === c2cid);

  // c1 取消关注
  c1.send({ type:'follow' });
  await sleep(300);
  const fs2 = c2.msgs.filter(m => m.type==='follow_stop' && m.from===c1cid).pop();
  ok('c2 收到 follow_stop', !!fs2);

  // 关注不存在用户 -> error
  const beforeErr = c1.errs;
  c1.send({ type:'follow', toId:'nope' });
  await sleep(300);
  const err = c1.msgs.filter(m=>m.type==='error').slice(beforeErr).pop();
  ok('关注不存在用户报错 no_such_user', err && err.code === 'no_such_user');

  server.kill();
  console.log(`\n[CollabBoard follow] pass=${pass} fail=${fail}`);
