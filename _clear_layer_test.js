// _clear_layer_test.js — CollabBoard clear_layer（房主或图层作者清空指定图层）
...
const PORT = 8097;
const ROOM = 'clrlay_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_clrlay_inline.js');
...
(async ()=>{
  ...
  // c2 在 L1 放两个元素
  c2.send({ type:'stroke', stroke:{ type:'stroke', layerId:'L1', points:[{x:1,y:1},{x:2,y:2}], color:'#fff' } });
  c2.send({ type:'note', text:'note on L1', x:5, y:5 });
  await sleep(300);

  // 房主清空 L1
  c1.send({ type:'clear_layer', layerId:'L1' });
  await sleep(300);
  const cleared = c2.msgs.filter(m => m.type === 'layer_cleared' && m.layerId === 'L1').pop();
  ok('c2 收到 layer_cleared(L1)', !!cleared && cleared.removed >= 1);
  ok('广播含 by=房主', cleared && cleared.by === c1cid);

  // 用 request_snapshot 验证 L1 已无残留
  c1.send({ type:'request_snapshot' });
  await sleep(250);
  const snap = c1.msgs.filter(m => m.type === 'snapshot').pop();
  ok('快照已无 L1 元素', snap && Array.isArray(snap.strokes) && !snap.strokes.some(el => el.layerId === 'L1'));

  // c1 在 L2 放元素（作者=c1）
  c1.send({ type:'stroke', stroke:{ type:'stroke', layerId:'L2', points:[{x:3,y:3},{x:4,y:4}], color:'#0f0' } });
  await sleep(250);

  // 非房主且非作者的 c2 清空 L2 => 报错
  const beforeErr = c2.msgs.filter(m => m.type === 'error').length;
  c2.send({ type:'clear_layer', layerId:'L2' });
  await sleep(250);
  const err = c2.msgs.filter(m => m.type === 'error').slice(beforeErr).pop();
  ok('非房主清空他人图层报错 not_owner', err && err.code === 'not_owner');

  // 缺 layerId => bad_layer
  const beforeBad = c1.msgs.filter(m => m.type === 'error').length;
  c1.send({ type:'clear_layer', layerId:'' });
  await sleep(250);
  const bad = c1.msgs.filter(m => m.type === 'error').slice(beforeBad).pop();
  ok('缺 layerId 报错 bad_layer', bad && bad.code === 'bad_layer');

  server.kill();
  console.log(`\n[CollabBoard clear_layer] pass=${pass} fail=${fail}`);
