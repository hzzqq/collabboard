// CollabBoard 端到端测试：验证编组 group / 解组 ungroup（服务端权威设置 group 字段、
// 广播 replace、落盘；重复编组为空操作；锁定房间非房主被拒）。
...
if(m){ const tmp = path.join(dir, '.wb_grp_inline.js'); fs.writeFileSync(tmp, m[1]);
...
(async ()=>{
  ...
  await sleep(150);
  const room = 'grpR_' + crypto.randomBytes(3).toString('hex');
  ...
  // 编组 a,b
  c1.send({ type:'group', ids:['a','b'], group:'g1' });
  await sleep(300);
  snap = await snapshotOf(room);
  const ga = snap.find(s=>s.id==='a'), gb = snap.find(s=>s.id==='b'), gc = snap.find(s=>s.id==='c');
  ok('a 被标记 group=g1', ga && ga.group === 'g1');
  ok('b 被标记 group=g1', gb && gb.group === 'g1');
  ok('c 未编组', gc && gc.group === undefined);

  // 广播 replace 含 group（c2 视角）
  const c2 = await connect(room);
  let replaced = null; c2.onmsg = msg=>{ if(msg.type==='replace' && Array.isArray(msg.strokes)) replaced = msg; };
  c1.send({ type:'group', ids:['c'], group:'g2' });
  await sleep(300);
  const rc = replaced && replaced.strokes.find(s=>s.id==='c');
  ok('广播 replace 含 c 的 group=g2', !!rc && rc.group === 'g2');

  // 解组 g1(a,b)
  c1.send({ type:'ungroup', ids:['a','b'] });
  await sleep(300);
  snap = await snapshotOf(room);
  ok('a 解组后无 group', !snap.find(s=>s.id==='a').group);
  ok('b 解组后无 group', !snap.find(s=>s.id==='b').group);

  // 重复编组空操作（已解组的 a 重新编入 g3）
  c1.send({ type:'group', ids:['a'], group:'g3' });
  await sleep(300);
  ok('a 重新编组为 g3', (await snapshotOf(room)).find(s=>s.id==='a').group === 'g3');

  // 锁定房间：非房主 group 被拒
  c1.send({ type:'lock', locked:true });
  await sleep(200);
  const c3 = await connect(room);
  let gotErr = false; c3.onmsg = msg=>{ if(msg.type==='error' && msg.code==='locked') gotErr = true; };
  c3.send({ type:'group', ids:['b'], group:'gX' });
  await sleep(300);
  ok('锁定房间非房主 group 被拒(error locked)', gotErr);
  const afterLock = await snapshotOf(room);
  ok('被拒后 b 仍无 group', !afterLock.find(s=>s.id==='b').group);

  server.kill();
  console.log(`\n[CollabBoard group] pass=${pass} fail=${fail}`);
