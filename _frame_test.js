// _frame_test.js — CollabBoard frame（带标题的画布分区/分组容器）
...
const PORT = 8131;
const ROOM = 'frame_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_frame_inline.js');
...
(async ()=>{
  ...
  // c1 建分区，c2 应收到 frame
  c1.send({ type:'frame', label:'Section A', x:40, y:40, w:320, h:240, color:'#82aaff' });
  await sleep(300);
  const fr = c2.msgs.filter(m => m.type==='frame' && m.label==='Section A').pop();
  ok('c2 收到 frame', !!fr);
  ok('frame 含服务端署名 author=c1', fr && fr.author === c1cid);
  ok('frame 含唯一 id', fr && typeof fr.id === 'string' && fr.id.length > 0);

  // 缺 label -> error
  const beforeErr = c1.errs;
  c1.send({ type:'frame', x:1, y:1, w:10, h:10 });
  await sleep(300);
  const err = c1.msgs.filter(m=>m.type==='error').slice(beforeErr).pop();
  ok('缺 label 报错 bad_label', err && err.code === 'bad_label');

  server.kill();
  console.log(`\n[CollabBoard frame] pass=${pass} fail=${fail}`);
