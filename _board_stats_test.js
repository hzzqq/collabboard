// _board_stats_test.js — CollabBoard board_stats（看板统计/可观测性）
...
const PORT = 8136;
const ROOM = 'bstat_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_bstat_inline.js');
...
(async ()=>{
  ...
  // 初始统计
  c1.send({ type:'board_stats' });
  await sleep(300);
  const s0 = c1.msgs.filter(m => m.type==='board_stats').pop();
  ok('收到 board_stats', !!s0);
  ok('clients>=2', s0 && s0.clients >= 2);
  ok('strokes 为数字', s0 && typeof s0.strokes === 'number');
  ok('locked 为布尔', s0 && typeof s0.locked === 'boolean');
  ok('password 为布尔', s0 && typeof s0.password === 'boolean');
  ...
  c1.send({ type:'board_stats' });
  await sleep(300);
  const s1 = c1.msgs.filter(m => m.type==='board_stats').pop();
  ok('一笔后 strokes===1', s1 && s1.strokes === 1);

  server.kill();
  console.log(`\n[CollabBoard board_stats] pass=${pass} fail=${fail}`);
