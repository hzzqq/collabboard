// _lock_board_test.js — CollabBoard lock_board（房主 toggle 整板锁定）
...
const PORT = 8098;
const ROOM = 'lkbrd_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_lkbrd_inline.js');
...
(async ()=>{
  ...
  // 房主锁定
  c1.send({ type:'lock_board' });
  await sleep(300);
  const lk = c2.msgs.filter(m => m.type === 'board_locked').pop();
  ok('c2 收到 board_locked(locked:true)', !!lk && lk.locked === true);
  ok('发送者 c1 也收到', c1.msgs.some(m => m.type === 'board_locked' && m.locked === true));

  // 再次切换 -> 解锁
  c1.send({ type:'lock_board' });
  await sleep(300);
  const un = c2.msgs.filter(m => m.type === 'board_locked').pop();
  ok('再次切换 board_locked(locked:false)', !!un && un.locked === false);

  // 非房主锁定 => not_owner，且不产生 board_locked 广播
  const beforeCnt = c1.msgs.filter(m => m.type === 'board_locked').length;
  const beforeErr = c2.msgs.filter(m => m.type === 'error').length;
  c2.send({ type:'lock_board' });
  await sleep(300);
  const err = c2.msgs.filter(m => m.type === 'error').slice(beforeErr).pop();
  ok('非房主 lock_board 报错 not_owner', err && err.code === 'not_owner');
  const afterCnt = c1.msgs.filter(m => m.type === 'board_locked').length;
  ok('非房主操作未触发 board_locked 广播', afterCnt === beforeCnt);

  server.kill();
  console.log(`\n[CollabBoard lock_board] pass=${pass} fail=${fail}`);
