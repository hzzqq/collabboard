// _cursor_toggle_test.js — CollabBoard cursor_toggle（显示/隐藏自己光标）
...
const PORT = 8094;
const ROOM = 'curtog_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_curtog_inline.js');
...
(async ()=>{
  ...
  // c2 隐藏光标 -> c1 收到 cursor_toggle(cid=c2, visible=false)
  c2.send({ type:'cursor_toggle', visible:false });
  await sleep(300);
  const t1 = c1.msgs.filter(m => m.type === 'cursor_toggle' && m.cid === c2cid).pop();
  ok('c1 收到 c2 隐藏光标事件', t1 && t1.visible === false);
  ok('事件含正确 cid', t1 && t1.cid === c2cid);

  // c2 再显示 -> visible=true
  c2.send({ type:'cursor_toggle', visible:true });
  await sleep(300);
  const t2 = c1.msgs.filter(m => m.type === 'cursor_toggle' && m.cid === c2cid).pop();
  ok('c1 收到 c2 显示光标事件', t2 && t2.visible === true);

  // 反向：c1 隐藏 -> c2 收到
  c1.send({ type:'cursor_toggle', visible:false });
  await sleep(300);
  const t3 = c2.msgs.filter(m => m.type === 'cursor_toggle' && m.cid === c1cid).pop();
  ok('c2 收到 c1 隐藏光标事件', t3 && t3.visible === false);

  // 非布尔 visible 被忽略（不产生该值）
  const before = c1.msgs.filter(m => m.type === 'cursor_toggle').length;
  c2.send({ type:'cursor_toggle', visible:'yes' });
  await sleep(200);
  const bad = c1.msgs.filter(m => m.type === 'cursor_toggle' && m.visible === 'yes');
  ok('非布尔 visible 被忽略', bad.length === 0);

  // 普通 cursor 移动仍可用（回归）
  c2.send({ type:'cursor', x:10, y:20 });
  await sleep(200);
  ok('普通 cursor 仍广播', c1.msgs.some(m => m.type === 'cursor' && m.x === 10 && m.y === 20));

  server.kill();
  console.log(`\n[CollabBoard cursor_toggle] pass=${pass} fail=${fail}`);
