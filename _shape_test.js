// _shape_test.js — CollabBoard shape（矢量图形基本图元 rect/ellipse/line/triangle）
...
const PORT = 8130;
const ROOM = 'shape_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_shape_inline.js');
...
(async ()=>{
  ...
  // c1 画矩形，c2 应收到 shape（除发送者外的广播）
  c1.send({ type:'shape', kind:'rect', x:10, y:20, w:100, h:50, color:'#ff0000' });
  await sleep(300);
  const rect = c2.msgs.filter(m => m.type==='shape' && m.shapeKind==='rect' && m.x===10).pop();
  ok('c2 收到 shape(rect)', !!rect);
  ok('shape 含服务端署名 author=c1', rect && rect.author === c1cid);
  ok('shape 含唯一 id', rect && typeof rect.id === 'string' && rect.id.length > 0);

  // 坐标钳制：超大 x 应被截断到 100000（隐性问题修复：防止非法坐标污染状态）
  c1.send({ type:'shape', kind:'ellipse', x:999999999, y:5, w:30, h:30, color:'#00ff00' });
  await sleep(300);
  const clamped = c2.msgs.filter(m => m.type==='shape').pop();
  ok('超大坐标被钳制到 100000', clamped && clamped.x === 100000);

  // 非法 kind -> 发送者收到 error
  const beforeErr = c1.errs;
  c1.send({ type:'shape', kind:'bogus', x:1, y:1, w:10, h:10 });
  await sleep(300);
  const e = c2.msgs.length, err = c1.msgs.filter(m=>m.type==='error').slice(beforeErr).pop();
  ok('非法 kind 报错 bad_kind', err && err.code === 'bad_kind');

  server.kill();
  console.log(`\n[CollabBoard shape] pass=${pass} fail=${fail}`);
