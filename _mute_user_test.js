// _mute_user_test.js — CollabBoard mute_user（房主禁言指定用户）
...
const PORT = 8096;
const ROOM = 'muteu_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_muteu_inline.js');
...
(async ()=>{
  ...
  // 房主禁言 c2
  c1.send({ type:'mute_user', toId:c2cid });
  await sleep(300);
  const muted = c2.msgs.filter(m => m.type === 'muted' && m.cid === c2cid).pop();
  ok('c2 收到 muted(cid=c2)', !!muted);
  ok('发送者 c1 也收到 muted 事件', c1.msgs.some(m => m.type === 'muted' && m.cid === c2cid));

  // 被禁言者发聊天 => 拦截
  const beforeChat = c2.msgs.filter(m => m.type === 'error').length;
  c2.send({ type:'chat', text:'hello from muted' });
  await sleep(300);
  const chatErr = c2.msgs.filter(m => m.type === 'error').slice(beforeChat).pop();
  ok('被禁言者聊天被拦截(错误 muted)', chatErr && chatErr.code === 'muted');
  // 房主仍能聊天
  c1.send({ type:'chat', text:'host ok' });
  await sleep(250);
  ok('房主聊天不被拦截', !!c2.msgs.some(m => m.type === 'chat' && m.text === 'host ok'));

  // 非房主禁言 => not_owner
  const beforeNo = c2.msgs.filter(m => m.type === 'error').length;
  c2.send({ type:'mute_user', toId:c1cid });
  await sleep(250);
  const no = c2.msgs.filter(m => m.type === 'error').slice(beforeNo).pop();
  ok('非房主 mute_user 报错 not_owner', no && no.code === 'not_owner');

  // 禁言自己 => mute_self
  const beforeSelf = c1.msgs.filter(m => m.type === 'error').length;
  c1.send({ type:'mute_user', toId:c1cid });
  await sleep(250);
  const self = c1.msgs.filter(m => m.type === 'error').slice(beforeSelf).pop();
  ok('房主禁言自己报错 mute_self', self && self.code === 'mute_self');

  server.kill();
  console.log(`\n[CollabBoard mute_user] pass=${pass} fail=${fail}`);
