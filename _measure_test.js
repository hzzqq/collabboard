// _measure_test.js — CollabBoard measure（两点距离测量，瞬时转发、不持久化）
...
const PORT = 8132;
const ROOM = 'meas_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_meas_inline.js');
...
(async ()=>{
  ...
  // 合法测量：3-4-5 直角三角形距离应为 50
  c1.send({ type:'measure', ax:0, ay:0, bx:30, by:40 });
  await sleep(300);
  const m = c2.msgs.filter(x => x.type==='measure').pop();
  ok('c2 收到 measure', !!m);
  ok('measure 距离=50', m && m.dist === 50);
  ok('measure 携带作者名', m && typeof m.name === 'string');

  // 非法坐标：不应广播任何 measure（隐性问题：NaN 距离被拦截，不污染他人画布）
  const before = c2.msgs.filter(x => x.type==='measure').length;
  c1.send({ type:'measure', ax:'xx', ay:0, bx:10, by:10 });
  await sleep(300);
  const after = c2.msgs.filter(x => x.type==='measure').length;
  ok('非法坐标不广播 measure', after === before);

  server.kill();
  console.log(`\n[CollabBoard measure] pass=${pass} fail=${fail}`);
