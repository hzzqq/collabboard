// _apply_template_test.js — CollabBoard apply_template（应用内置白板/四象限模板）
...
const PORT = 8137;
const ROOM = 'tmpl_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_tmpl_inline.js');
...
(async ()=>{
  ...
  // 应用 brain 模板
  c1.send({ type:'apply_template', name:'brain' });
  await sleep(300);
  const rep = c2.msgs.filter(m => m.type==='replace').pop();
  ok('c2 收到 replace（模板元素）', !!rep);
  const frames = (rep && rep.strokes ? rep.strokes : []).filter(s => s.type==='frame');
  ok('模板含 4 个 frame', frames.length === 4);
  ok('frame 含标签“想法”', frames.some(f => f.label === '想法'));
  ok('frame 含作者署名', frames.every(f => typeof f.author === 'string'));

  // 未知模板 -> error（隐性问题：仅允许名单内模板，防任意元素注入）
  const beforeErr = c1.errs;
  c1.send({ type:'apply_template', name:'../../../etc' });
  await sleep(300);
  const err = c1.msgs.filter(m=>m.type==='error').slice(beforeErr).pop();
  ok('未知模板报错 bad_template', err && err.code === 'bad_template');

  server.kill();
  console.log(`\n[CollabBoard apply_template] pass=${pass} fail=${fail}`);
