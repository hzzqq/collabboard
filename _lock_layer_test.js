// _lock_layer_test.js — CollabBoard lock_layer（房主锁定/解锁图层）
...
const PORT = 8099;
const ROOM = 'lklay_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_lklay_inline.js');
...
(async ()=>{
  ...
  // 房主锁定图层
  c1.send({ type:'lock_layer', layerId:'LX' });
  await sleep(300);
  const lk = c2.msgs.filter(m => m.type === 'layer_locked' && m.layerId === 'LX').pop();
  ok('c2 收到 layer_locked(LX, locked:true)', !!lk && lk.locked === true);
  ok('广播含 by=房主', lk && lk.by === c1cid);

  // 解锁
  c1.send({ type:'lock_layer', layerId:'LX', locked:false });
  await sleep(300);
  const un = c2.msgs.filter(m => m.type === 'layer_locked' && m.layerId === 'LX').pop();
  ok('再次 layer_locked(LX, locked:false)', !!un && un.locked === false);

  // 非房主 => not_owner
  const beforeErr = c2.msgs.filter(m => m.type === 'error').length;
  c2.send({ type:'lock_layer', layerId:'LZ' });
  await sleep(300);
  const err = c2.msgs.filter(m => m.type === 'error').slice(beforeErr).pop();
  ok('非房主 lock_layer 报错 not_owner', err && err.code === 'not_owner');

  // 缺 layerId => bad_layer
  const beforeBad = c1.msgs.filter(m => m.type === 'error').length;
  c1.send({ type:'lock_layer', layerId:'' });
  await sleep(250);
  const bad = c1.msgs.filter(m => m.type === 'error').slice(beforeBad).pop();
  ok('缺 layerId 报错 bad_layer', bad && bad.code === 'bad_layer');

  server.kill();
  console.log(`\n[CollabBoard lock_layer] pass=${pass} fail=${fail}`);
