// _save_view_test.js — CollabBoard save_view/load_view（个人视口保存与跳转）
...
const PORT = 8134;
const ROOM = 'sview_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_sview_inline.js');
...
(async ()=>{
  ...
  // c1 保存视口
  c1.send({ type:'save_view', x:50, y:60, zoom:2 });
  await sleep(300);
  const sv = c2.msgs.filter(m => m.type==='view_saved' && m.id===c1cid).pop();
  ok('c2 收到 view_saved', !!sv);
  ok('view_saved 坐标/缩放正确', sv && sv.x===50 && sv.y===60 && sv.zoom===2);

  // c2 加载 c1 的视口
  c2.send({ type:'load_view', id:c1cid });
  await sleep(300);
  const v = c2.msgs.filter(m => m.type==='view' && m.id===c1cid).pop();
  ok('c2 加载到 c1 视口', !!v);
  ok('view 坐标/缩放一致', v && v.x===50 && v.y===60 && v.zoom===2);

  // 加载不存在的视口 -> error
  const beforeErr = c2.errs;
  c2.send({ type:'load_view', id:'ghost' });
  await sleep(300);
  const err = c2.msgs.filter(m=>m.type==='error').slice(beforeErr).pop();
  ok('加载不存在视口报错 no_view', err && err.code === 'no_view');

  server.kill();
  console.log(`\n[CollabBoard save_view] pass=${pass} fail=${fail}`);
