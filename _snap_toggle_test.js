// _snap_toggle_test.js — CollabBoard snap_toggle（房主切换吸附网格）
...
const PORT = 8101;
const ROOM = 'snap_t_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
(async ()=>{
  ...
  // 房主开吸附
  c1.send({ type:'snap_toggle', on:true });
  await sleep(300);
  const on = c2.msgs.filter(m => m.type === 'snap_toggle').pop();
  ok('c2 收到 snap_toggle(on:true)', !!on && on.on === true);
  ok('广播含 by=房主', on && on.by === c1cid);

  // 关吸附
  c1.send({ type:'snap_toggle', on:false });
  await sleep(300);
  const off = c2.msgs.filter(m => m.type === 'snap_toggle').pop();
  ok('snap_toggle(on:false)', !!off && off.on === false);

  // 无参切换：false -> true
  c1.send({ type:'snap_toggle' });
  await sleep(300);
  const t = c2.msgs.filter(m => m.type === 'snap_toggle').pop();
  ok('无参 snap_toggle 切换为 true', !!t && t.on === true);

  // 非房主 => not_owner
  const beforeErr = c2.msgs.filter(m => m.type === 'error').length;
  c2.send({ type:'snap_toggle', on:true });
  await sleep(300);
  const err = c2.msgs.filter(m => m.type === 'error').slice(beforeErr).pop();
  ok('非房主 snap_toggle 报错 not_owner', err && err.code === 'not_owner');

  server.kill();
  console.log(`\n[CollabBoard snap_toggle] pass=${pass} fail=${fail}`);
