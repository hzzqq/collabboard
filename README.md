...
- **元素编组（group / ungroup）**：把多个选中元素编为同一 `groupId`，可整体移动 / 旋转 / 删除 / 复制，客户端新增编组 / 解组按钮（ci108）。
- **对齐工具（align）**：选中多个元素按 左 / 水平居中 / 右 / 上 / 垂直居中 / 下 六种方式对齐，服务端权威重排并广播（ci112）。
- **Ping 提醒（ping）**：对选中元素发起 ping，他人画布上该元素高亮闪烁提示（临时转发、不落库、不受房间锁限制），便于在协作中聚焦注意力（ci116）。
- **房主转移（transfer）**：房主可将 owner 转移给其他在线用户，服务端校验目标有效后广播 `owner` 变更；非房主 / 无效目标返回 `error`（ci120）。
- **远端选中态同步（select-presence）**：实时把本端选中元素 id 集合广播给其他人，他人画布显示彩色虚框高亮；`presence` 消息携带来宾 id 列表，离场自动清理远端选中框（ci124）。
...
# _group_test      (12/12) 元素编组：groupId 绑定 / 整体移动 / 解组 / 撤销
# _align_test      (14/14) 对齐工具：左/中/右/上/中/下六种 / 服务端重排 / 广播
