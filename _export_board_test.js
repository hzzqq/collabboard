// _export_board_test.js — CollabBoard export_board（导出整板 json/svg，含完整元数据）
...
const PORT = 8141;
const ROOM = 'exp_' + crypto.randomBytes(3).toString('hex');
let fail = 0, pass = 0;
...
if(sc){
  const tmp = path.join(dir, '.wb_exp_inline.js');
...
(async ()=>{
  ...
  // 先画一笔，便于验证导出内容完整
  c1.send({ type:'stroke', stroke:{ type:'stroke', points:[{x:1,y:1},{x:2,y:2}], color:'#fff' } });
  await sleep(300);

  // 导出 JSON
  c1.send({ type:'export_board', format:'json' });
  await sleep(300);
  const ej = c1.msgs.filter(m => m.type==='board_export' && m.format==='json').pop();
  ok('收到 json 导出', !!ej);
  ok('导出含 strokes 数组', ej && Array.isArray(ej.data.strokes));
  ok('导出 strokes 含刚才一笔', ej && ej.data.strokes.length === 1);
  ok('导出含元数据(bg/title/permissions)', ej && 'bg' in ej.data && 'title' in ej.data && 'permissions' in ej.data);

  // 导出 SVG
  c1.send({ type:'export_board', format:'svg' });
  await sleep(300);
  const es = c1.msgs.filter(m => m.type==='board_export' && m.format==='svg').pop();
  ok('收到 svg 导出', !!es);
  ok('svg 内容非空', es && typeof es.svg === 'string' && es.svg.length > 0);
  ok('svg 为合法根元素', es && es.svg.startsWith('<svg') && es.svg.endsWith('</svg>'));

  // 非法格式 -> error
  const beforeErr = c1.errs;
  c1.send({ type:'export_board', format:'xml' });
  await sleep(300);
  const err = c1.msgs.filter(m=>m.type==='error').slice(beforeErr).pop();
  ok('非法导出格式报错 bad_format', err && err.code === 'bad_format');

  server.kill();
  console.log(`\n[CollabBoard export_board] pass=${pass} fail=${fail}`);
